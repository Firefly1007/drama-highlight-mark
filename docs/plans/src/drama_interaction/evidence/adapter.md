...
- 读取一集 V1 片段 JSON 和起始节点提供的集长，构造并落盘一个 EvidenceDocument。
...
- 当前起始节点取合法 `end` 的最大值作为集长；未来 FFmpeg 阶段负责探测并向上取整。适配器只校验并写入 episode_duration_ms。
...
- 片段时间必须与状态提供的集长一致；任何超界时间都作为上游数据错误暴露。

## 失败与边界情形

- 输入 JSON 缺失、字段非法、时间无法解析或 episode_duration_ms 缺失/非法时快速失败；适配器不从 V1 时间线推导替代集长，当前临时来源仅属于起始节点。
