...
- execution 保存 execution_id、输入路径、状态和时间戳；`drama_context` 保存按剧名加载的剧名、简介和角色表；episode_duration_ms 保存起始节点提供的集长。当前来源为 V1 片段最大 `end`，未来替换为 FFmpeg 结果。
- evidence 保存只读的 EvidenceDocument；derived_evidence 按 specialist_type 分组保存派生观察。
- specialist_results、candidate_pool、manual_candidates、repair_log、hitl_queue、constraints_report、scheduler_decision、rendered_candidates、selected_candidate_ids、final_interactions 和 metrics 保存各阶段产物。
...
- 状态、修复记录和指标以可序列化形式写入 PostgreSQL checkpoint，恢复后保留完整上下文。

## 失败与边界情形

- 不允许不同集、不同 execution_id 或不同分支的状态数据混写。
- 基线证据不得被服务、专家或修复流程改写。
- 具体容器形态、指标和 HITL 字段由状态模型与 [ADR](../../../../drama-interaction-v2-ADR.md) 共同约束；运行状态见 [PROGRESS](../../../../PROGRESS.md)。
